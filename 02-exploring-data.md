...
![](fig/openrefine-create-project.png){alt='OpenRefine Create Project'}
...
4. On the next screen, OpenRefine will present you with a preview of your data. You can check here for obvious errors, if, for example, your file was tab-delimited rather than comma-delimited, the preview would look strange (and you
  ...
  ![](fig/openrefine-data-import.png){alt='OpenRefine Import Data'}

5. In the middle of the page, will be a set of options (`Character encoding`, etc.). Make sure the tick box next to
...
2. Click the down arrow and choose `Facet` > `Text facet`.
  ...
  ![](fig/ORFacetMenu.png){alt='OpenRefine Facet menu'}

3. In the left panel, you'll now see a box containing every unique value in the `scientificName` column
  along with a number representing how many times that value occurs in the column.
  
  ![](fig/ORFacetedScientificName.png){alt='Faceting results on scientificName column'}
...
3. By default, the column `yr` is formatted as Text. You can change the format by doing `Edit cells` > `Common transforms` >
